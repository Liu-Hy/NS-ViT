"""Script to train on the HAL cluster"""
...
def prepare_loader(split_data, info_path, batch_size, transform=None):
    ...
    sampler = torch.utils.data.distributed.DistributedSampler(split_data)
    data_loader = DataLoader(split_data, batch_size=batch_size, sampler=sampler, num_workers=8, pin_memory=True)
    return data_loader
...
def main(gpu, args):
    ...
    val_batch_size = 128
    rounds = 3
    lr = wandb.config.lr  # When using SGD and StepLR, set to 0.001 # when AdamW and bachsize=256, 3e-4
    lim = wandb.config.lim
    nlr = wandb.config.nlr
    eps = wandb.config.eps
    adv = True
    img_ratio = 0.01 #0.02
    train_ratio = 0.01 #0.1
    val_ratio = 0.01 #0.05
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if adv:
            ...
            if Path.exists(noise_path.joinpath(str(epoch))):
                ...
                delta_x = torch.load(noise_path.joinpath(str(epoch)))['delta_x'].cuda(gpu)
            else:
