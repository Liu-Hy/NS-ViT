...
def predict_save(mod_img, fname='sample'):
  ...
  view_mod_img = torch.cat([fn(mod_img[0], 0).unsqueeze(0), fn(mod_img[1], 1).unsqueeze(0), fn(mod_img[2], 2).unsqueeze(0)])
...
iclr_img = normalize(transforms.ToTensor()(Image.open(BytesIO(response.content)).convert('RGB').resize((img_size, img_size))))
