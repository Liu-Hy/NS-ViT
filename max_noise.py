...
def predict(net, mod_img, img, mean, std):
  ...
  vmod = torch.cat([fn(mod_img[0], 0).unsqueeze(0), fn(mod_img[1], 1).unsqueeze(0), fn(mod_img[2], 2).unsqueeze(0)], dim=0)
...
def main(model_name, img_size=224, lp=2):
  ...
  result = torch.load(f'./outputs/{model_name}.robust.patch')
  for lim in [0.01]:
